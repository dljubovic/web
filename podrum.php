...
?>  
    ...
    <?php if (isset($_GET['actionaddProduct']) || isset($_GET['filter']) || isset($_GET['orderby'])) : ?>
    ...
    <main>
        ...
        <div class="filter-order-container">
            <div class="filter-order-title">
                <div class="filter">
                    <span class="filter-title">FILTRIRAJ VINA</span><br><br>
                    <div class="filter-forms">
                        <form action="<?php echo "?filter=" . filterValueWhite(); if (isset($_GET['orderby'])) { echo "&orderby=" . $_GET['orderby']; } ?>" method="post">
                            <button class="filter-button <?php if (isset($_GET['filter']) && strpos($_GET['filter'], 'white') !== false) echo "filter-active"?>" type="submit">BIJELA</button>    
                        </form>
                        <form action="<?php echo "?filter=" . filterValueRed(); if (isset($_GET['orderby'])) { echo "&orderby=" . $_GET['orderby']; } ?>" method="post">
                            <button class="filter-button <?php if (isset($_GET['filter']) && strpos($_GET['filter'], 'red') !== false) echo "filter-active"?>" type="submit" name="filter_button_r">CRNA</button>
                        </form>
                        <form action="<?php echo "?filter=" . filterValueOther(); if (isset($_GET['orderby'])) { echo "&orderby=" . $_GET['orderby']; } ?>" method="post">
                            <button class="filter-button <?php if (isset($_GET['filter']) && strpos($_GET['filter'], 'other') !== false) echo "filter-active"?>" type="submit" name="filter_button_o">OSTALO</button>
                        </form>
                    </div>
                </div><br>
                <div class="order">
